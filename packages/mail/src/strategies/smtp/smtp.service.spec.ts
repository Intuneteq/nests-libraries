...
describe('SmtpMailStrategy', () => {
  ...
  it('throws when setOptions receives incomplete config', () => {
    const strategy = new SmtpMailStrategy(mailOptions as any)
  ...
  it('sends a transformed smtp message after initialization', async () => {
    ...
    expect(sendMail).toHaveBeenCalledWith(mail.getSmtpMessage(mailOptions.from))
  })

  it('throws when send is called before initialization', async () => {
    const strategy = new SmtpMailStrategy(mailOptions as any)

    await expect(strategy.send(new TestMailable())).rejects.toThrow(new HttpException('SMTP configuration not set', 500))
  })
})
