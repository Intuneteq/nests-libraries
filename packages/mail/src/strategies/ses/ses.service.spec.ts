...
jest.mock('@aws-sdk/client-sesv2', () => {
  ...
  const command = jest.fn().mockImplementation((input) => ({ input }))
...
describe('SesMailStrategy', () => {
  ...
  it('sends a transformed SES message after initialization', async () => {
    ...
    expect(sendEmailCommand).toHaveBeenCalledWith(mail.getSesMessage(mailOptions.from))
    expect(send).toHaveBeenCalledWith({ input: mail.getSesMessage(mailOptions.from) })
  })
  ...
  it('wraps provider errors in an internal server error', async () => {
    ...
    await expect(strategy.send(new TestMailable())).rejects.toThrow(InternalServerErrorException)
  })
})
