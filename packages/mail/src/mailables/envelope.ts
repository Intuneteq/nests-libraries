...
export class Envelope {
    ...
    constructor(data?: EnvelopeType) {
        if (data) {
            if (data.from)
                this.from = this.normalizeToAddressArray(data.from)[0]
            ...
            if (data.replyTo)
                this.replyTo = this.normalizeToAddressArray(data.replyTo)
            if (data.inReplyTo)
                this.inReplyTo = this.normalizeToAddressArray(data.inReplyTo)[0]
     ...
     */
    private normalizeToAddressArray(
        ...
        if (Array.isArray(input)) {
            return input.map((i) =>
                typeof i === "string" ? new Address(i) : i
            )
        }
        return []
    }
